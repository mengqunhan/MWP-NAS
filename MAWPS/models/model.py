...
class UniversalModel(BertPreTrainedModel):
    def __init__(self, config: BertConfig,
        ...
        # 选择数字的格式
        # self.num_format_classifier = nn.Sequential(
        ...
        #     nn.Linear(args.hidden_size // 4, args.num_format_num)
        # )

        # self.candidate_ffn=nn.ModuleList()
        # for i in range(self.num_start):
        #     # + * *- +/ PAD
        #     self.candidate_ffn.append(
        #         nn.Sequential(
        #             nn.Linear(args.hidden_size,args.hidden_size),
        #             nn.ReLU(),
        #             nn.LayerNorm(args.hidden_size,eps=args.layer_norm_eps),
    ...
    def evalue(self,
        ...
        self.end_idx = end_idx
        predict_mtree = self.recover_mtree(predict, predict_format, max_height, max_width, 1, 0)
        node_right, format_right, mtree_right, result_batch, predict_mtrees = self.mtree_equalbatch(predict_mtree,
                                                                                                    target_mtree)
        return node_right, format_right, mtree_right, result_batch, predict_mtrees
    ...
    def mtree_equalbatch(self, mtree1_batch, mtree2_batch):
        ...
        for i in range(batch_size):
            ...
            result.append({
                ...
                'format_flag': format_flag,
                'mtree_flag': mtree_flag})
        return node_right, format_right, mtree_right, result, predict_mtrees
        ...
        if pad_all:
            ...
            if data==pad_idx or data>=self.num_start or data==end_idx:
                ...
                if len(mtree['children'])==0:
                    ...
                    end_flag=False #相当于mtree['children'][-1]['data']
                    for child in mtree['children']:
                        if end_flag:
                            child['data']=pad_idx
