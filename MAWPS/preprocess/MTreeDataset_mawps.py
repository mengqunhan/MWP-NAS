...
class MTreeDataset(Dataset):
    def __init__(self, args, tokenizer, file, number, mode) -> None:
    ...
    def read_math23k_file(self,file):
        ...
        for obj in tqdm(data,desc='Tokenization',total=len(data),ncols=80):
            try:
                mtree,num_to_code,code_to_num = exp_to_mtree(obj['processed_equation'],self.old_idx2word,self.old_num_start)
            except:
                default_pair+=1
                continue
            try:
            ...
            mapped_text = obj['processed_text']
            ...
            if len(var_starts)!=len(obj['num_list']):
                align_default+=1
                continue
        ...
        print(f"Align default: {align_default}")
        print(f"Width default: {width_default}")
        print(f"Order default: {order_default}")
    ...
    def pad_mtree(self,mtree,max_height,max_width):
        ...
        if max_height==1:
            ...
            if len(mtree['children'])==0:
                ...
                mtree['children'].extend([{'data':pad_token,'children':[]} for _ in range(max_width-len(mtree['children']))])
            for child in mtree['children']:
